...
to-report load-table-betaDist
  ...
  set demoTable map [ i -> i * betaDist-av-num-offspring / (length filter [j -> j > 0] demoTable) ] demoTable
...
to-report load-table-cdemodel
  ...
  [
    i ->
    set demoTable lput (
      cde-alpha * exp(cde-beta * (i - cde-delta) - exp(cde-gamma * (i - cde-delta)))
    ) demoTable
  ]
